...
* 分词器默认字典路径
*/
  private static final String PATH_DIC_MAIN = "dict/dict-excel-1.dic";
  private static final String PATH_DIC_QUANTIFIER = "dict/quantifier.dic";

  /*
   * 分词器配置文件路径
   */
  private static final String FILE_NAME = "config/IKAnalyzer.cfg.xml";
  // 配置属性——扩展字典
  ...
  public String getMainDictionary() {
    /*Resource resource = new ClassPathResource(PATH_DIC_MAIN);
    try {
  ...
  public String getQuantifierDicionary() {
    /*Resource resource = new ClassPathResource(PATH_DIC_QUANTIFIER);
    try {
      System.out.println(resource.getURL().getPath());
      return resource.getURL().getPath();
    } catch (IOException e) {
